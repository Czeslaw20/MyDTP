...
int TcpSocket::connectTimeout(sockaddr_in *addr, unsigned int wait_seconds)
{
    ...
    //非阻塞模式连接，返回-1，并且errno=EINPROGRESS，表示连接正在进行中
    if (ret < 0 && errno == EINPROGRESS)
    {
        ...
        do
        {
            //一旦连接建立，则套接字就可写，所以connect_fdset放在了写集合中
...
*@buf:接收缓冲区
*@count:要读取的字节数
*成功返回count，失败返回-1，独到EOF返回<count
*/
