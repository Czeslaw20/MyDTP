...
Cryptographic::Cryptographic(string filename, bool isPrivate = true)
{
    m_publicKey = RSA_new();
    m_privateKey = RSA_new();
    if (isPrivate)
...
//生成RSA密钥对
void Cryptographic::generateRsaKey(int bits, string pub = "public.pem",
...
//使用RSA签名
string Cryptographic::rsaSign(string data, SignLevel level = Level3)
{
    unsigned int len;
...
//使用RSA验证签名
bool Cryptographic::rsaVerify(string data, string signData,
